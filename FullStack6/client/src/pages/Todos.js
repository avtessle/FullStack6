...
function Todos() {
    ...
    if (editingTodoId != null) {
      try {
        ...
        const response = await fetch(
          `http://localhost:3000/todos/update-title/${editingTodoId}`,
          {
            method: "PUT",
            headers: {
              "Content-Type": "application/json",
  ...
  const handleCheckChange = async (id, title, completed) => {
    try {
      ...
      const response = await fetch(
        `http://localhost:3000/todos/update-completed/${id}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
            ...
            <div className={styles["todos-item"]} key={todo.id}>
              <div className={styles["todo-case"]}>
                <input
                  ...
                  onChange={() =>
                    handleCheckChange(todo.id, todo.title, !todo.completed)
                  }
                />
                ...
                <button
                  onClick={() =>
                    handleSave(todo.id, todo.title, todo.completed)
                  }
                >
                  <FaSave />
                </button>
